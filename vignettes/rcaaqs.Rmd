...
Now we can pass this on to the `caaqs_management()`function via the `exclude_df` 
...
Note that there are some new columns in the caaqs data frame: `excluded`, 
`mgmt_metric_value`, and `mgmt_level`. The `excluded` column has some `TRUE` 
values, reflecting the fact that some dates in that metric have been excluded. 
(Note that excluding data in this manner does not influence the evaluation of data completeness).
...
```{r}
pm_az2 <- airzone_metric(pm_az, caaqs_val = "ambient_metric_value", 
                         caaqs = "ambient_caaqs", mgmt = "mgmt_level")
pm_az2
```
...
g1 <- ggplot() +
  geom_sf(data = az_plot, aes(fill = ambient_caaqs)) +
  geom_sf(data = st_as_sf(pm_az, coords = c("lon", "lat"), crs = "+proj=longlat"),
          shape = 21, colour = "black", aes(fill = ambient_caaqs)) +
