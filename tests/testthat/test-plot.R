...
test_that("plot_ts fails correctly", {
  names(daily_data)[2:3] <- c("date", "avg24h")
  ...
  names(daily_data)[2:3] <- c("foo", "avg24h")
  ...
  expect_error(plot_ts(daily_data, caaqs_data = NULL, parameter = "pm2.5_24h", 
                       rep_yr = 2013, plot_exceedances = FALSE))
  # Wrong data formats
  names(daily_data) <- c("date", "foo", "avg24h", "n_readings") # date is character
  expect_error(plot_ts(daily_data, caaqs_data = NULL, parameter = "pm2.5_24h", 
                       rep_yr = 2013, plot_exceedances = FALSE))
  names(daily_data) <- c("avg24h", "date", "foo", "n_readings") # avg24h is character
  expect_error(plot_ts(daily_data, caaqs_data = NULL, parameter = "pm2.5_24h", 
                       rep_yr = 2013, plot_exceedances = FALSE))
})
...
test_that("works with caaqs_data (pm24h)", {
  names(daily_data) <- c("id", "date", "avg24h", "n_readings")
  caaqs_data <- pm_24h_caaq(annual_data)
