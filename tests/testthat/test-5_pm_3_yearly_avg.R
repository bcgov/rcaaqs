...
test_that("Runs with silently", {
  expect_silent(r1 <- pm_yearly_avg(pm1))
  expect_silent(r2 <- pm_yearly_avg(pm2, by = c("ems_id", "site")))
  saveRDS(r1, "pm_yearlyavg_1.rds")
  saveRDS(r2, "pm_yearlyavg_2.rds")
})

ret1 <- readRDS("pm_yearlyavg_1.rds")
ret2 <- readRDS("pm_yearlyavg_2.rds")
...
test_that("performs data completeness accurately", {
  for(r in list(ret1, ret2)){
    ...
    expect_true(all(is.na(r$ann_avg[(r$quarter_1 < 0.6 |
                                       r$quarter_2 < 0.6|
                                       r$quarter_3 < 0.6|
                                       r$quarter_4 < 0.6)])))
  }
...
test_that("can exclude data rows", {
  ...
  high_dates <- dplyr::filter(pm2, avg_24h > get_std("pm2.5_annual")) %>%
    dplyr::select(ems_id, site, date)
  
  expect_silent(ret3 <- pm_yearly_avg(pm2, by = c("ems_id", "site"),
