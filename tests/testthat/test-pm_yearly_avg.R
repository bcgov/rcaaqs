context("pm 98 yearly average")

multi_id <- readRDS("daily_averages.rds")
one_id <- multi_id[multi_id$id == "a", ]

test_one <- pm_yearly_avg(one_id, dt = "dates", val = "val")
test_mult <- pm_yearly_avg(multi_id, dt = "dates", val = "val", by = "id")
...
test_that("can exclude data rows", {
  ...
  ret <- pm_yearly_avg(multi_id, dt = "dates", val = "val", by = "id", 
                            exclude_df = excl_df, exclude_df_dt = c("start", "stop"))

  expect_equal(ret$ann_avg[ret$id == "a" & ret$year == 2011], 4.2)
})
