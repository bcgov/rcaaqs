...
test_one <- pm_yearly_avg(one_id)
test_mult <- pm_yearly_avg(multi_id, by = "ems_id")

test_that("can exclude data rows", {
  # Take out values 4.0 or above to make sure it changes
  high_dates <- 
    multi_id$date[multi_id$ems_id == "0220205" & strftime(multi_id$date,"%Y") == 2011 & 
                    !is.na(multi_id$avg_24h)  & multi_id$avg_24h >= 4.0]
  excl_df <-
  ...
  ret <- pm_yearly_avg(multi_id, by = "ems_id", 
                       exclude_df = excl_df, exclude_df_dt = c("start", "stop"))

  expect_equal(ret$ann_avg[ret$ems_id == "0220205" & ret$year == 2011], 2.00)
})
