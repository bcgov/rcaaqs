...
annual_values <- readRDS("pm_24h_y98.rds")
annual_values_one_id <- annual_values[annual_values$ems_id == "0220205", ]

test_mult <- pm_three_yr_avg(annual_values[, -2], by = "ems_id")
test_one <- pm_three_yr_avg(annual_values_one_id[, c(-1, -2)])
...
test_that("Has correct dimensions", {
  expect_equal(dim(test_mult), c(24, 15))
  expect_equal(dim(test_one), c(1, 14))
})
...
test_that("n_years is correct", {
  ## removing a row
  ret <- pm_three_yr_avg(annual_values[-c(3:5), -2], by = "ems_id")
