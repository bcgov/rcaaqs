context("so2 98 percentile")

multi_id <- readRDS("daily_averages.rds")
one_id <- multi_id[multi_id$id == "a", ]

test_one <- so2_yearly_99(one_id, dt = "dates", val = "val")
test_mult <- so2_yearly_99(multi_id, dt = "dates", val = "val", by = "id")
...
test_that("Only accepts date objects", {
  ...
  expect_error(so2_yearly_99(test, "posix", "val"))
})
...
test_that("Has the right column names and dimensions", {
  expected_names <-c("year", "valid_in_year", "quarter_1", "quarter_2", "quarter_3", 
                     "quarter_4", "ann_99_percentile", "valid_year", "exceed", 
...
test_that("Exceed works", {
  ...
  res <- so2_yearly_99(multi_id, dt = "dates", val = "val", by = "id")
...
test_that("can exclude data rows", {
  ...
  ret <- so2_yearly_99(multi_id, dt = "dates", val = "val", by = "id", 
                       exclude_df = excl_df, exclude_df_dt = c("start", "stop"))
  
  expect_equal(ret$ann_99_percentile[ret$id == "a" & ret$year == 2011], 8.6)
})
